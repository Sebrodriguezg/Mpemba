...
% ============================================================ JUAN T1
\blk{\Jb}{1 · Predicción del cruce (sin evolucionar)}{0:30}
\begin{itemize}
  \item ``Lo bonito: con el espectro y \emph{una traza} ya predecimos quién hará
    Mpemba, sin resolver la dinámica. Dos estados: $|+\rangle^{\otimes N}$ (parte lejos)
    y $|0\rangle^{\otimes N}$ (parte cerca).''
  \item ``El de lejos arranca más alto pero pesa \textbf{menos} en el modo lento
    ($a_2$: 0.13 vs 0.50) $\Rightarrow$ termina por debajo $\Rightarrow$ \textbf{cruce
    garantizado}, $t^*\approx0.68$. T2 y T3 solo lo confirman.''
\end{itemize}
