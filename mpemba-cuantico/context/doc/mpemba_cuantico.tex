...
\boxed{\;
\rho_t=e^{t\Lop}[\rho_0]
=\rss+\sum_{k=2}^{d^2}\, a_k\, e^{\lambda_k t}\,\hat r_k \;}
\label{eq:spectral}
\end{equation}
donde el coeficiente
\begin{equation}
a_k\equiv\Tr\!\big(\hat l_k^{\dagger}\rho_0\big)
\label{eq:overlap}
\end{equation}
es el \emph{solapamiento inicial} del estado $\rho_0$ con el $k$-ésimo modo
(obtenido proyectando $\rho_0=\sum_k a_k\hat r_k$ con la biortonormalidad
\eqref{eq:biorth}). Cada $\hat r_k$ ($k\ge2$) es un \emph{modo de excitación} que
decae con tasa $|\!\operatorname{Re}(\lambda_k)|$.
...
\subsection{¿OpenMP, MPI o CUDA? Paradigma óptimo por tarea}
\label{sec:paradigma_por_tarea}

La elección del modelo de paralelización no es arbitraria: se \emph{deduce} del
patrón de cómputo y del patrón de acceso a memoria de cada tarea. Tres patrones,
tres protagonistas:
\begin{itemize}[leftmargin=1.6em,itemsep=1pt]
\item \textbf{MPI} (memoria distribuida) domina donde el factor limitante es la
\emph{memoria agregada} ($\mathrm{nnz}(\mathbb{L})\sim4^N$) o donde hay
\emph{reducciones globales} frecuentes (productos internos, promedios).
\item \textbf{CUDA} (GPU) domina donde el cuello de botella es el \emph{ancho de
banda de memoria} sobre datos regulares con alta reutilización del operador
(SpMV repetido, GEMM).
\item \textbf{OpenMP} (memoria compartida) es la \emph{capa intranodo
transversal}: acelera el SpMV y las factorizaciones dentro de cada rango, y es el
\emph{fallback} en nodos sin GPU.
\end{itemize}
La regla práctica: el grano grueso (entre nodos) lo fija MPI; el grano fino
(dentro del nodo) lo fija CUDA si los datos son regulares, OpenMP si no.

\paragraph{(T1) Modos lentos $\;\to\;$ MPI (híbrido MPI${+}$OpenMP).}
El cómputo es Arnoldi con \emph{shift-invert}: ortogonalización de Gram--Schmidt
---productos internos con \textbf{all-reduce} global en cada iteración--- más una
\emph{resolución lineal dispersa} $(\mathbb{L}-\sigma\mathbb{1})^{-1}$ con factores
LU que no caben en un nodo. El limitante es, por tanto, memoria distribuida ${+}$
comunicación colectiva: territorio nativo de \textbf{MPI} (\textsc{slepc/petsc}
con \textsc{mumps}/\textsc{superlu\_dist}). OpenMP entra \emph{dentro} de cada
rango para el SpMV y la factorización. CUDA aporta poco aquí: la resolución
lineal dispersa irregular y la dependencia secuencial del Gram--Schmidt encajan
mal en GPU (salvo como precondicionador puntual).

\paragraph{(T2) Evolución temporal $\;\to\;$ CUDA (multi-GPU vía MPI si no cabe).}
La acción de Krylov / RK4 son \emph{miles de SpMV repetidos sobre el mismo}
$\mathbb{L}$: cómputo regular, limitado por ancho de banda, con máxima
reutilización del operador. Es el caso de uso estrella de \textbf{CUDA}
(\textsc{cuSPARSE}): manteniendo $\mathbb{L}$ residente en VRAM y encadenando los
pasos temporales \emph{sin trasiego} CPU$\leftrightarrow$GPU se obtiene el mayor
retorno por nodo (típicamente ${\sim}$1 orden de magnitud en ancho de banda
frente a CPU). \textbf{MPI} solo aparece cuando $m\,d^2$ excede una sola GPU:
entonces se particiona el vector entre varias GPUs con intercambio de halo.
OpenMP es el \emph{fallback} en nodos sin acelerador (es lo que hace hoy el
núcleo de referencia en C++).

\paragraph{(T3a) Trayectorias cuánticas $\;\to\;$ MPI (${+}$ \emph{batching} en CUDA).}
Son $M$ evoluciones \emph{independientes} de vectores pequeños
$\ket{\psi}\in\mathbb{C}^{d}$, sin comunicación salvo la reducción final
$\rho=\mathbb{E}[\dyad{\psi}{\psi}]$. Es el caso \emph{embarrassingly parallel}
por excelencia: \textbf{MPI} reparte lotes de trayectorias entre nodos con una
única \texttt{MPI\_Reduce} al final, y la precisión escala como $M^{-1/2}$ con
eficiencia casi perfecta. \textbf{CUDA} es el acelerador ideal \emph{dentro} de
cada rango: una GPU evoluciona cientos--miles de trayectorias en paralelo
(\emph{batched state-vector}), pues cada una es ligera. El esquema óptimo es
\textbf{jerárquico}: MPI entre nodos, CUDA (o OpenMP, como el prototipo actual con
\texttt{multiprocessing}) dentro.

\paragraph{(T3b) Redes tensoriales $\;\to\;$ CUDA.}
Las contracciones de MPO/MPDO son \emph{GEMM por bloques} de coste ${\sim}\chi^3$:
la dimensión de enlace $\chi$ se traduce en matrices densas grandes. Ese es el
\emph{workload} donde la GPU es imbatible (\textsc{cuBLAS}/\textsc{cuTENSOR});
\textbf{CUDA} es la elección. \textbf{MPI} distribuye los bloques de la cadena o
barre $\chi$/parámetros; OpenMP queda relegado a fallback.
...
\begin{tabular}{@{}lllll@{}}
\toprule
Tarea & Patrón de cómputo & Cuello de botella & Primario & Apoyo \\
\midrule
T1 modos lentos & Arnoldi ${+}$ solve disperso & all-reduce ${+}$ memoria & \textbf{MPI} & OpenMP \\
T2 evolución    & SpMV repetido ($e^{t\mathbb L}v$) & ancho de banda & \textbf{CUDA} & MPI multi-GPU \\
T3a trayectorias& $M$ evoluciones indep.\ de $\ket\psi$ & ninguno (independientes) & \textbf{MPI} & CUDA \emph{batch} \\
T3b redes tens. & GEMM por bloques ($\chi^3$) & cómputo denso & \textbf{CUDA} & MPI \\
\bottomrule
\end{tabular}
\caption{Paradigma de paralelización óptimo por tarea. El primario fija el grano
grueso; el apoyo acelera el grano fino o amplía la memoria. OpenMP es además el
\emph{fallback} sin GPU en todas las tareas.}
\label{tab:paradigma}
\end{table}

\paragraph{Camino de implementación por fases.}
Las tareas se introducen en un orden que maximiza el valor incremental y da
\emph{variedad} de paradigmas (cada fase estrena uno):
\begin{enumerate}[leftmargin=1.8em,itemsep=2pt]
\item[\textbf{F0}] \emph{Referencia (hecho).} Núcleo C++ con OpenMP (SpMV
intranodo) ${+}$ MPI (barrido de preparaciones). Valida la física y fija la
\emph{baseline} de rendimiento.
\item[\textbf{F1}] \emph{T2 en CUDA.} Portar el SpMV a \textsc{cuSPARSE} y mantener
$\rho$ en VRAM. Mayor ganancia por nodo y primer salto en el $N$ alcanzable.
\item[\textbf{F2}] \emph{T1 en MPI.} Acoplar \textsc{slepc/petsc} con
\emph{shift-invert} para los modos lentos a escala multi-nodo; reutiliza el SpMV
\emph{matrix-free} de F0 como operador.
\item[\textbf{F3}] \emph{T3a en MPI (${+}$ CUDA).} Trayectorias repartidas entre
nodos con reducción final; \emph{batching} en GPU dentro de cada rango. Habilita
$N$ donde $d^2$ ya no cabe.
\item[\textbf{F4}] \emph{T3b en CUDA.} Redes tensoriales (\textsc{cuTENSOR}) para
cadenas largas y la asimetría de entrelazamiento.
\end{enumerate}

\begin{cajaclave}
\textbf{Síntesis.} No hay un paradigma único óptimo: el \emph{valor} del proyecto
está en que cada tarea exhibe el suyo. \textbf{MPI} para lo dominado por memoria
agregada o reducciones (T1, T3a); \textbf{CUDA} para lo dominado por ancho de
banda con datos regulares (T2, T3b); \textbf{OpenMP} como capa intranodo y
\emph{fallback} universal. La arquitectura final es \emph{híbrida y jerárquica}:
MPI entre nodos $\Rightarrow$ CUDA/OpenMP dentro.
\end{cajaclave}
...
\subsection{Serie vs paralelo: medición directa de la mejora}

Los entregables de evolución temporal (T2) y trayectorias (T3) miden la mejora de
forma explícita, enfrentando el \textbf{tiempo de cómputo en serie y en paralelo}
en función del parámetro propio de cada método. Las dos curvas de cada panel~(a)
hacen visible la ganancia como la separación entre ellas, y el panel~(b) muestra
el speedup $T_{\rm serie}/T_{\rm par}$ resultante.

La integración temporal con OpenMP (RK4 y Krylov, Figuras~\ref{fig:sp_t2a}
y~\ref{fig:sp_t2b}) paraleliza el producto de matrices \emph{dentro} de cada
paso: el speedup \emph{crece con la malla} $d=2^N$ —de $\sim\!1{,}6$ a $\sim\!4{,}4$
al pasar de $N=4$ a $N=7$, pues a mayor $d$ el \texttt{matmul} domina y amortiza el
\emph{fork/join}— pero satura por debajo del ideal (memoria compartida,
\emph{bandwidth-bound}). Las trayectorias con MPI (Figura~\ref{fig:sp_t3})
paralelizan \emph{entre} tareas independientes: las rectas serie y paralelo
\emph{divergen} al crecer el trabajo $M$ y el speedup es \emph{casi constante}
($\approx5{,}6$, cerca del número de núcleos), la firma del paralelismo
\emph{embarrassingly parallel}. Es la ilustración cuantitativa de que el paradigma
óptimo lo dicta el patrón de cómputo, no una preferencia a priori.

\begin{figure}[h]
\centering
\includegraphics[width=0.86\textwidth]{../../T2/a_integracion_directa/results/fig_t2a_serial_parallel.png}
\caption{T2(a) RK4 directo (OpenMP): tiempo serie ($1$~hilo) vs paralelo
($8$~hilos) frente al tamaño del sistema $N$ (malla $d=2^N$). El speedup crece con
la malla y satura sublineal.}
\label{fig:sp_t2a}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.86\textwidth]{../../T2/b_accion_exponencial/results/fig_t2b_serial_parallel.png}
\caption{T2(b) Krylov (OpenMP): mismo patrón que RK4 —el hotspot paralelo, el
\texttt{matmul} de la acción de $\mathcal L$, es común a ambos métodos.}
\label{fig:sp_t2b}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.86\textwidth]{../../T3/integration/results/fig_t3_serial_parallel.png}
\caption{T3(a) trayectorias cuánticas (MPI): tiempo serie ($1$~rango) vs paralelo
($8$~rangos) frente al número de trayectorias $M$. Las rectas divergen y el speedup
es casi constante ($\approx5{,}6$): escalado casi ideal, sin comunicación.}
\label{fig:sp_t3}
\end{figure}
