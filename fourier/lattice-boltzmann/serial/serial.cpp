/**
 * @file serial.cpp
 * @brief Simulador LBM D1Q3 Serial para el efecto Mpemba macroscópico
 */
...
#include <fstream>
#include <iomanip>
#include <string> // Necesario para manipular el nombre del archivo

// --- Parámetros Computacionales y Físicos ---
constexpr int N_NODES = 1000;
constexpr double DX = 1e-4;
constexpr double DT = 1e-3;
constexpr int MAX_STEPS = 1000000; // 100 segundos físicos
constexpr int OUTPUT_FREQ = 500;
...
int main(int argc, char* argv[]) {
    ...
    std::string filename = "datos-serial_T" + std::to_string(static_cast<int>(T_INIT)) + ".csv";
    ...
    out_file.close();
    std::cout << "-> LBM finalizado para T_INIT = " << T_INIT << "C. Archivo: " << filename << std::endl;
    return 0;
}
