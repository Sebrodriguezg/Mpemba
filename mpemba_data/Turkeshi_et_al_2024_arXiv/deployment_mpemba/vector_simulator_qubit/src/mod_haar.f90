module mod_haar
  ...
  type, extends(basis) :: circuit 
    ...
    complex(dp), dimension(4,4) :: gate
    integer, allocatable, dimension(:) :: shiftr, shiftl, shift2r
    type(basis), allocatable, dimension(:) :: idx_rho_aq
  contains 
    ...
    procedure :: get_asymmetry_distances
    procedure :: get_entanglement_asymmetry 
  ...
  ! circuit contructor 
  function circuit_constructor(lsys,na) result(self)
    type(circuit) :: self 
    type(basis) :: parent  
    integer :: lsys, na 
    ...
    do s=1,self%size 
      self%shiftr(s) = ishftc(s-1,1,lsys)+1
      self%shiftl(s) = ishftc(s-1,-1,lsys)+1
      self%shift2r(s) = ishftc(s-1,2,lsys)+1
    end do
  ...
  subroutine set_initial_theta_state(self,theta)
    ...
    associate(lsys=>self%lsys, psi=>self%psi,temp=>self%temp,size=>self%size)
      ...
      do i=1,size 
        m = zero 
        do a=0,lsys-1 
          if (btest(i-1,a)) m = m+one 
        end do 
        psi(i) = cos(theta/two)**(m)*(-sin(theta/two))**(real(lsys,dp)-m)
      end do 
    end associate
  ...
  !      m2 = zero 
  !      do a=0,lsys-1,2
  !        if (btest(i-1,a)) m1 = m1+one 
  !      end do 
  !      do a=1,lsys-1,2
  !        if (btest(i-1,a)) m2 = m2+one 
  !      end do 
  !      psi(i) = cos(theta/two)**(m1)*(-sin(theta/two))**(real(lsys/2,dp)-m1)* &
  ...
  subroutine get_asymmetry_distances(self,asym) 
    class(circuit), intent(in out) :: self 
  ...
  end subroutine get_asymmetry_distances

  ! get asymmetry 
  subroutine get_entanglement_asymmetry(self,asym) 
    class(circuit), intent(in out) :: self 
    real(dp), dimension(2), intent(in out):: asym 
  ...
  end subroutine get_entanglement_asymmetry 
  ...
  subroutine apply_u1_gate_and_shift(self)
    ...
    associate(temp=>self%temp,psi=>self%psi,size=>self%size,gate=>self%gate,t2=>self%shift2r)
      gate = zero 
      call generate_cue(2,gate(2:3,2:3))
      ...
      do ii=1,4
        ib = (size/4)*(ii-1)
        do a =1, size/4 
          temp(ib+a) = sum(gate(ii,:)*psi(a:size:mm))
        end do 
      end do 
      psi = temp(t2)
    end associate 
  ...
  subroutine apply_u1_unitary_layer(self,it)
    ...
    associate(lsys=>self%lsys,temp=>self%temp,psi=>self%psi,&
      ...
      if (mod(it,2)==1) psi = psi(t1r)
      do el = 1,lsys,2
        call self%apply_u1_gate_and_shift()
      end do 
      if (mod(it,2)==1) psi = psi(t1) 
    end associate
