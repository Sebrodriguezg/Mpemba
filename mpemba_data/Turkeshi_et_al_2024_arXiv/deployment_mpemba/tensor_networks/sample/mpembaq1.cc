...
int main(int argc, char* argv[]){
  ...
  const std::vector<double> Fdn ={1,0,0,1,1,1};
  ...
  std::string filename = "/home/xhek.turkeshi/itensor/dev/T2_q1.csv";
  ...
  for (int i=1; i<tmax; i++){
    for (int NA=1; NA<=L/2; NA++) {
      initializeBoundary(bnd, locdim, NA, L, Fup, Fdn);
      double pur_a = -std::log(inner(bnd,psi));
      std::complex<double> val = 0.0;
      for (int k=0; k<=NA; k++){
        double kk = PI*2.0*k/((double) NA+1);
        const std::vector<std::complex<double>> Fdnk ={1,0,0,exp(II*kk),exp(-II*kk),1};
        initializeBoundary(bndk, locdim, NA, L, Fup, Fdnk);
