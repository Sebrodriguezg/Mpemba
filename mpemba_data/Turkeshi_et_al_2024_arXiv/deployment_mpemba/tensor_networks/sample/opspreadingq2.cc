...
ITensor createGate(const MPS& psi, int site1, int site2, const std::vector<std::vector<double>>& uq, int locdim){
  auto gate = ITensor(siteIndex(psi, site1), siteIndex(psi, site2),prime(siteIndex(psi, site1)), prime(siteIndex(psi, site2)));
...
template <typename T>
void initializeBoundary(MPS& bnd, int locdim, int NA, int L, const std::vector<double>& Vac, const std::vector<T>& Doped){
  auto setter_first = ITensor(siteIndex(bnd, 1), rightLinkIndex(bnd, 1));
  for (int n = 1; n <= locdim; ++n) {
    setter_first.set(n, 1, Vac[n - 1]);
  }
  ...
  for (int k = 2; k <= L-1; ++k) {
    ...
    if (k==NA) {
      for (int n = 1; n <= locdim; ++n) {
        setter.set(1, n, 1, Doped[n - 1]);
      }
    } else {
      for (int n = 1; n <= locdim; ++n) {
        setter.set(1, n, 1, Vac[n - 1]);
      }
    }
    bnd.set(k, setter);
  }
...
void applyGateAndSVD(MPS& psi, const ITensor& gate, int site1, int site2, double CUTOFF, int MAX_DIM){
  psi.position(site1);
...
int main(int argc, char* argv[]){
  ...
  const double costh2 = cos(th/2.0)*cos(th/2.0);
  const double sinth2 = sin(th/2.0)*sin(th/2.0);
  const double q=2.0;
  const int tmax=120;
  
  auto sites = CustomSpin(L,{"S=", 3.5,  "ConserveQNs=",false});
  auto init = InitState(sites);

  std::vector<std::vector<double>> uq = readCSV("../WW2.csv");

  const std::vector<double> Vac ={1, 1, 1, 1, 0, 0, 0, 0};
  const std::vector<double> DN ={0, 0, 0, 0, 0, 0, 1.0/q, 0};
  const std::vector<double> Theta ={costh2*costh2,costh2*sinth2,costh2*sinth2,sinth2*sinth2,costh2*costh2,costh2*sinth2,costh2*sinth2,sinth2*sinth2};

  auto psi = MPS(init);
  auto bnd = MPS(init);

  initializeBoundary(bnd, locdim, NA, L, Vac, DN);
  initializePsi(psi, locdim, L, Theta);
  ...
  for (int i=1; i<tmax; i++){
    ...
    std::cout<< i << "\t"<< pur_a  << "\t" << maxLinkDim(psi) << std::endl;
    for (int k = 1; k < L / 2; ++k) {
      int site1 = 2 * k;
      int site2 = (2 * k) % L + 1;
      int gateIndex = k + L / 2 - 1;
      applyGateAndSVD(psi, gates[gateIndex], site1, site2, 1E-20, MAX_DIM);
    }
    for (int k = 1; k <= L / 2; ++k) {
      int site1 = 2 * k - 1;
      int site2 = 2 * k;
