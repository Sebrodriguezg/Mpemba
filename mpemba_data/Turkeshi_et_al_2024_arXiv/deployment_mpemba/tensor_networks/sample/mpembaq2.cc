...
void initializeBoundary(MPS& bnd, int locdim, int NA, int L, const std::vector<double>& Vac, const std::vector<T>& Doped){
  ...
  for (int n = 1; n <= locdim; ++n) {
    setter_first.set(n, 1, Doped[n - 1]);
  }
...
int main(int argc, char* argv[]){
  ...
  const std::vector<double> Vac ={1,0,0,0,0,0,0,0};
  const std::vector<double> Doped ={0,1,0,0,0,0,0,0};
  const std::vector<double> Theta ={1,1,cos(th),cos(th),cos(th),cos(th),cos(th)*cos(th),cos(th)*cos(th)};
  ...
  for (int i=1; i<tmax; i++){
    for (int NA=1; NA<=L/2; NA++) {
      initializeBoundary(bnd, locdim, NA, L, Vac, Doped);
      double pur_a = -std::log(inner(bnd,psi));
      std::complex<double> val = 0.0;
      for (int k=0; k<=NA; k++){
        double kk = PI*2.0*k/((double) NA+1);
        const std::vector<std::complex<double>> Dopedk ={0,cos(kk)*cos(kk),0,II*cos(kk)*sin(kk),0,-II*cos(kk)*sin(kk),0,sin(kk)*sin(kk)};
        initializeBoundary(bndk, locdim, NA, L, Vac, Dopedk);
        val += innerC(bndk,psi)/((double) NA+1);
      }
      double pur_aq = -std::log(std::real(val));
      std::cout<< L << "," << i << ","<< NA<< ","<< pur_a << "," << pur_aq << "," << maxLinkDim(psi) << std::endl;
    }
    for (int k = 1; k < L / 2; ++k) {
      int site1 = 2 * k;
      int site2 = (2 * k) % L + 1;
      int gateIndex = k + L / 2 - 1;
      applyGateAndSVD(psi, gates, site1, site2, gateIndex, 1E-10, MAX_DIM);
    }
    for (int k = 1; k <= L / 2; ++k) {
      int site1 = 2 * k - 1;
      int site2 = 2 * k;
